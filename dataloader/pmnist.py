...
def get(seed=0, fixed_order=False, pc_valid=0.1):
    ...
    nperm = 10 #10 tasks
    seeds = np.array(list(range(perm)), dtype=int)
    if not fixed_order:
    ...
    if not os.path.isdir(pmnist_dir):
        ...
        std = (0.3801,)
        dat = {}
        dat['train'] = datasets.MNIST(mnist_dir, train=True, download=True,
                        transform=transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean, std)]))
        dat['test'] = datasets.MNIST(mnist_dir, train=False, download=True,
                        transform=transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean, std)]))
    ...
    #Validation
    for t in data.keys():
        r=np.arrange(data[t]['train']['x'].size(0))
